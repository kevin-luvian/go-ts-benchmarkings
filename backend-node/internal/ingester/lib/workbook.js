...
function XlsxStreamReaderWorkBook(options) {
  ...
  if (!(this instanceof XlsxStreamReaderWorkBook))
    return new XlsxStreamReaderWorkBook(options);
    ...
    srcPipe
      ...
      .on("entry", function (entry) {
        ...
        switch (entry.path) {
          ...
          default:
            if (
              (match = entry.path.match(/xl\/(worksheets\/sheet(\d+)\.xml)/i))
            ) {
              var sheetPath = match[1];
              var sheetNo = match[2];

              if (
                self.parsedWorkBookInfo === false ||
...
XlsxStreamReaderWorkBook.prototype._parseXML = function (
  entryStream,
  entryHandler,
  endHandler
) {
  ...
  parser.on("closetag", function (nodeName) {
    ...
    if (tmpNodeEmit) {
      ...
      tmpNode.length &&
      tmpNode[tmpNode.length - 1] &&
      tmpNode[tmpNode.length - 1].name === nodeName
    ) {
      tmpNode.push("");
      entryHandler.call(self, tmpNode);
      tmpNodeEmit = false;
...
XlsxStreamReaderWorkBook.prototype._parseSharedStrings = function (nodeData) {
  ...
  if (isSharedStringItem) {
    ...
    if (nodeObjName && nodeObjName.name === "t") {
      self.workBookSharedStrings[self.workBookSharedStrings.length - 1] +=
        nodeObjValue;
    }
  }
};
...
XlsxStreamReaderWorkBook.prototype._parseWorkBookInfo = function (nodeData) {
  ...
  nodeData.forEach(function (data) {
    if (data.name === "sheet") {
      self.workBookInfo.sheetRelationshipsNames[data.attributes["r:id"]] =
...
XlsxStreamReaderWorkBook.prototype._parseWorkBookRels = function (nodeData) {
  ...
  nodeData.forEach(function (data) {
    if (data.name === "Relationship") {
      self.workBookInfo.sheetRelationships[data.attributes.Target] =
        data.attributes.Id;
    }
  });
};
