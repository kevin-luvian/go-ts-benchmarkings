...
function XlsxStreamReaderWorkSheet(
  workBook,
  sheetName,
  workSheetId,
  workSheetStream
  ...
  if (!(this instanceof XlsxStreamReaderWorkSheet))
    return new XlsxStreamReaderWorkSheet(
      workBook,
      sheetName,
      workSheetId,
      workSheetStream
...
XlsxStreamReaderWorkSheet.prototype._handleWorkSheetStream = function () {
  ...
  self.on("pipe", function (srcPipe) {
    self.workBook._parseXML.call(
      self,
      srcPipe,
      self._handleWorkSheetNode,
      function () {
...
XlsxStreamReaderWorkSheet.prototype._handleWorkSheetNode = function (nodeData) {
  ...
  if (self.inRows === true) {
          ...
          if (self.options.formatting && workingVal) {
            if (self.workBook.hasFormatCodes) {
              var formatId = workingCell.attributes.s
                ? self.workBook.xfs[workingCell.attributes.s].attributes
                    .numFmtId
                : 0;
    ...
    if (workingCell.name === "v") {
      const cellNum = self.getColumnNumber(self.currentCell.attributes.r);
